...
if type(doc.ActiveView) != Autodesk.Revit.DB.View3D:
    rpw.ui.forms.Alert('Active view is not 3D view')
    sys.exit()

rebars = select_rebars()

with rpw.db.Transaction('Unset Rebars Solid in Active View'):
    for rebar in rebars:
        rebar.SetSolidInView(doc.ActiveView, False)
